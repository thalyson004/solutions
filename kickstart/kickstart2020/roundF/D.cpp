/*
   Problem name: Yeetzhee
   Difficulty level: Medium-Hard
   Techniques: 
      Dynamic Programming
      Sorting
      Expected Value
      Searching with cut branches
   Hints:
      Uses Expected Value in Expected Value equation.
      Reverse vector for compare less in map<vector<int>, double>. 
...
double solve(vector<int> actual){
   sortv(actual);
   reversev(actual);
   if( dp.count(actual) ) return dp[actual];

   if( possible(actual, initial)==false ){
      return -1.0;
   }
   ...
   double valid = 0.0;
   for(int i=0; i < M; i++){
      int add = (i<actual.size());
      if(add) actual[i]++;
      else actual.push_back(1);
      ...
      if(add) actual[i]--;
      else actual.pop_back();
   }
   ans = ans/valid + double(M)/valid;
