/*
  Problem name: Painters' Duel 
  Difficulty level: Medium problem
  Techniques: DFS, BackTracking and Max(Min()) Game.
...
int inside(int r, int c){
   return r >=0 and r < N and c >= 0 and c < r+r+1 and G[r][c]==0;
}

int solve(int ra, int ca, int ma, int rb, int cb, int mb, int vez){

   if(ma==0 and mb==0 ) return 0;
   if(ma==0 and vez==0 ) return solve(0,0,0,rb,cb,mb,1);
   if(mb==0 and vez==1) return solve(ra,ca,ma,0,0,0,0);
   ...
   if(vez==0){
      ...
      for(int i = 0; i < 3; i++){
         ...
         if( inside(nr, nc) ){
            cm = 1;
            G[nr][nc]=1;
            ans = max(ans, solve(nr,nc,1,rb,cb,mb,!vez)+1 );
            G[nr][nc]=0;
         }
      }

      if( cm==0 )
         return solve(0,0,0, rb, cb, mb, 1);
      else
         return ans;
      ...
      for(int i = 0; i < 3; i++){
         ...
         if( inside(nr, nc) ){
            cm = 1;
            G[nr][nc]=1;
            ans = min(ans, solve(ra,ca,ma,nr,nc,1,!vez)-1 );
            G[nr][nc]=0;
         }
      }

      if( cm==0 )
         return solve(ra,ca,ma, 0, 0, 0, 0);
      else
         return ans;
   }
...
main(){
   ...
   for(int cases = 1; cases <= CASES; cases++){
      ...
      println( solve(ra,ca, 1, rb, cb, 1, 0) );
   }
}
