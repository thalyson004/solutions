...
main(){
   ...
   for(int cases = 1; cases <= CASES; cases++){
      ...
      PrefixSumBit bit(N+1);
      for(int i = 0; i < N; i++){
         bit.add(toys[i].e, toys[i].id);
      }
      priority_queue<toy> remain;
      for(int i = 0; i < N; i++) remain.push( toys[i] );
      ...
      while( removed.size() or remain.size() ){
         while( remain.size() and remain.top().e+remain.top().r>sum ){
            toy cur = remain.top();
            remain.pop();
            removed.push( -cur.id );
         }
         ...
         if(removed.size()==0){
            ans = oo;
            ansRem = N-remain.size();
            break;
         }

         int id = -removed.top();
         removed.pop();
         int curAns = sum + bit.preSum(id-1);
         if( ans < curAns ){
            ans = curAns;
            ansRem = rem;
         }
         int value = bit.preSum(id)-bit.preSum(id-1);
         sum -= value;
         bit.add(-value, id);
         rem++;
      }
      if(ans==oo)
         println(ansRem, "INDEFINITELY");
      else
         println(ansRem, ans);
