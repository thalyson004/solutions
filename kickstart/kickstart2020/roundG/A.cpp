/*
   ...
   Techniques: Prefixsum
   Hints: For O(N), think about prefixsum of KICK or posfixsum of START
   Written by Thalyson
*/
...
main(){
   ...
   for(int cases = 1; cases <= CASES; cases++){
      string s;
      cin >> s;
      string left = "KICK", right = "START";
      int start  = 0;
      int ans  = 0;
      for(int i = 0; i+right.size()-1<s.size(); i++){
         if( s.substr(i, right.size())==right ) ans+=start;
         if( s.substr(i, left.size())==left) start++;
      }
      cout << "Case #" << cases << ": ";
      println(ans);
   }
